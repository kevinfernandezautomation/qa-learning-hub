...
function renderAcademy(){
  ...
  const list=window.CERTIFICATIONS.filter(c=>{const levelMatch=!lev||c.level===lev||(lev==='Foundation'&&c.id==='AICS-ASTFC');const ws=window.CERT_WORK_SCENARIOS?.[c.id];return levelMatch&&(!dif||c.difficulty===dif)&&(!q||[c.id,c.name,c.focus,c.level,ws?.scenario,ws?.context,ws?.application].join(' ').toLowerCase().includes(q));});
  if(!grid)return;
  grid.innerHTML=list.map(c=>`<article class="cert-card"><div class="cert-top"><span class="badge">${escapeHtml(c.level)}</span><span class="status">${escapeHtml(c.status)}</span></div><h2>${escapeHtml(c.name)}</h2><p>${escapeHtml(c.focus)}</p><div class="cert-meta"><span><b>Dificultad</b>${escapeHtml(c.difficulty)}</span><span><b>Nivel</b>${escapeHtml(c.k)}</span><span><b>Examen</b>${escapeHtml(c.exam)}</span></div>${(()=>{const ws=window.CERT_WORK_SCENARIOS?.[c.id];return ws?`<details class="cert-work-scenario"><summary>Aplicación laboral · escenario realista</summary><div class="scenario-body"><h3>${escapeHtml(ws.scenario)}</h3><p><strong>Contexto:</strong> ${escapeHtml(ws.context)}</p><p><strong>Cómo aplicar el conocimiento:</strong> ${escapeHtml(ws.application)}</p><p><strong>Entregable esperado:</strong> ${escapeHtml(ws.deliverable)}</p></div></details>`:''})()}<div class="card-actions"><a class="btn primary small" href="simuladores.html?cert=${encodeURIComponent(c.id)}">Practicar</a><a class="btn secondary small" href="${c.id==='CTFL'?'ctfl-v4.html':'ruta-certificacion.html?cert='+encodeURIComponent(c.id)}">Ver ruta →</a>${c.id.startsWith('AICS')?`<a class="btn secondary small" href="${c.url}" target="_blank" rel="noopener">Fuente oficial ↗</a>`:`<a class="btn syllabus-btn small" href="${ISTQB_SYLLABUS_URLS[c.id]||c.url}" target="_blank" rel="noopener">Ver syllabus ↗</a>`}<a class="btn secondary small" href="cert-aplicacion.html?cert=${encodeURIComponent(c.id)}">Escenario paso a paso →</a><button class="btn secondary small add-cert-cart" type="button" data-course="Preparación ${escapeHtml(c.id)} — ${escapeHtml(c.name)}">Agregar al carrito</button></div></article>`).join('')||'<div class="empty-state"><h2>Sin resultados</h2><p>Cambie los filtros o la búsqueda.</p></div>';
  $$('.add-cert-cart',grid).forEach(b=>b.addEventListener('click',()=>{
    addAcademyCourseToCart(b.dataset.course);
    const status=$('#certCartStatus');if(status){status.hidden=true;status.textContent=''}
  }));
...
const STACK_LEARNING=[
 ...
 {id:"data",name:"QA de Bases de Datos · SQL + NoSQL",level:"Intermedio",modules:["Fundamentos de persistencia","SQL relacional","NoSQL documental","Web y mobile data","Integridad y esquema","Transacciones y concurrencia","UI/API/BD end-to-end","Seguridad y rendimiento"],desc:"Quality Assurance para bases relacionales y no relacionales: SQL Server, PostgreSQL, MySQL, Oracle, MongoDB, Firestore y SQLite."},
...
function renderExternalCertifications(){
 ...
 el.innerHTML=items.map(x=>`<article class="cert-card credential-card"><div class="cert-top"><span class="badge">Certificación externa</span><span class="status">${escapeHtml(x.provider)}</span></div><h2>${escapeHtml(x.name)}</h2><p>${escapeHtml(x.desc)}</p><div class="cert-meta"><span><b>Nivel</b>${escapeHtml(x.level)}</span><span><b>Modalidad</b>${escapeHtml(x.duration)}</span><span><b>Áreas</b>${x.modules.length}</span></div><ol class="module-list">${x.modules.map(m=>`<li>${escapeHtml(m)}</li>`).join('')}</ol><details class="cert-work-scenario"><summary>Aplicación laboral · ejemplo</summary><div class="scenario-body"><p><strong>Escenario:</strong> un equipo necesita aplicar ${escapeHtml(x.name)} dentro de una iniciativa real de QA.</p><p><strong>Cómo demostrarlo:</strong> use los módulos de la credencial (${x.modules.map(m=>escapeHtml(m)).join(", ")}) para resolver una tarea verificable y documentar evidencia.</p><p><strong>Entregable:</strong> evidencia técnica, resultados de prueba o automatización alineados al objetivo de la credencial.</p></div></details><div class="card-actions"><a class="btn primary small" href="${x.url}" target="_blank" rel="noopener">Ver certificación ↗</a><button class="btn secondary small add-external-cert-cart" type="button" data-course="Preparación ${escapeHtml(x.name)}">Agregar al carrito</button></div></article>`).join('');
