...
function buildDemoInvoice(method){
 ...
 localStorage.setItem('lastPayment',JSON.stringify({reference:`PAY-${Date.now()}`,status:'DEMO_PAID',method,total,createdAt:invoice.createdAt})); const hist=JSON.parse(localStorage.getItem('qaPaymentRecords')||'[]'); hist.push({...invoice,status:'PAID'}); localStorage.setItem('qaPaymentRecords',JSON.stringify(hist));
...
const LOCAL_PAYMENTS={CR:{name:'SINPE Móvil',detail:'Transferencia vinculada al número móvil mediante entidades participantes del Sistema Nacional de Pagos.'},CO:{name:'Bre-B',detail:'Pagos y transferencias inmediatas interoperables mediante llaves o QR en entidades participantes.'},BR:{name:'Pix',detail:'Pago instantáneo disponible 24/7 mediante llave Pix o QR en instituciones participantes.'},MX:{name:'CoDi / transferencia SPEI',detail:'Pago local mediante los mecanismos habilitados por la entidad receptora. Confirme los datos antes de transferir.'},AR:{name:'Transferencias 3.0 / QR',detail:'Pagos con transferencia inmediata y QR interoperable a través de bancos y billeteras participantes.'},PE:{name:'Transferencia inmediata / billetera local',detail:'Use el medio local indicado por la cuenta receptora; la interoperabilidad depende de las entidades participantes.'},ES:{name:'Bizum / transferencia inmediata',detail:'Use Bizum únicamente si la cuenta receptora lo tiene habilitado; de lo contrario utilice transferencia bancaria.'}};
function updateLocalPayment(){const c=$('#localPaymentCountry')?.value||'CR',x=LOCAL_PAYMENTS[c]||LOCAL_PAYMENTS.CR;if($('#localPaymentSystem'))$('#localPaymentSystem').value=x.name;if($('#localPaymentInfo'))$('#localPaymentInfo').innerHTML=`<strong>${escapeHtml(x.name)}</strong><p>${escapeHtml(x.detail)}</p>`;}
$('#localPaymentCountry')?.addEventListener('change',updateLocalPayment);updateLocalPayment();
$('#sinpeSubmit')?.addEventListener('click',()=>{if(!selected.size){stat.textContent='Seleccione al menos un curso antes de registrar el SINPE Móvil.';return;}const ref=$('#sinpeReference')?.value.trim();if(!ref){stat.textContent='Ingrese la referencia del comprobante.';return;}const localCountry=$('#localPaymentCountry')?.value||'CR',localMethod=LOCAL_PAYMENTS[localCountry]?.name||'Transferencia local',subtotal=cartSubtotal(),total=subtotal+(subtotal*VAT),hist=JSON.parse(localStorage.getItem('qaPaymentRecords')||'[]');hist.push({number:`LOCAL-${Date.now()}`,createdAt:new Date().toISOString(),method:localMethod,country:localCountry,customer:{name:$('#billingName')?.value.trim()||'Pendiente',email:$('#billingEmail')?.value.trim()||''},items:[...selected].map(name=>({name,price:itemByName(name)?.price||PRICE})),total,status:'PENDING_VERIFICATION',reference:ref});localStorage.setItem('qaPaymentRecords',JSON.stringify(hist));stat.textContent=`Pago por ${localMethod} registrado. El acceso quedará activo después de la verificación administrativa.`;});
