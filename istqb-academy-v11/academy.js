...
function renderLearningPaths(){
 ...
 el.innerHTML=window.LEARNING_PATHS.map(x=>{
   const game=x.id==='game-qa',virtuoso=x.id==='virtuoso-qa';
...
function renderLibrary(){if($('#ebookGrid'))$('#ebookGrid').innerHTML=window.EBOOKS.map(resourceCard).join('');if($('#magazineGrid'))$('#magazineGrid').innerHTML=window.MAGAZINES.map(resourceCard).join('');window.applyTranslations?.();}
